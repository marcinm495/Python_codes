...
from sympy import MatrixSymbol, Matrix, Identity, Sum, Symbol, sqrt
from cmath import exp, pi, e
...
class Bosonic_Tritters:
    ...
    def toKet3(self, state):
        A = MatrixSymbol('A', 3*self.n_systems, 1)
        state_out = state
        for k in range(0, 3*self.n_systems):
            for k1 in range(0, 3*self.n_systems):
                for k2 in range(0, 3*self.n_systems):
                    state_out = state_out.subs(A[k, 0]*A[k1, 0]*A[k2, 0], "|"+str(k % 3)+str(k1 % 3)+str(k2 % 3)+">")
        return state_out
    
    def toKet2(self, state):
        A = MatrixSymbol('A', 3*self.n_systems, 1)
        state_out = state
        for k in range(0, 3*self.n_systems):
            for k1 in range(0, 3*self.n_systems):
                state_out = state_out.subs(A[k, 0]*A[k1, 0], "|"+str(k % 3)+str(k1 % 3)+">")
        return state_out
    ...
    def postselectSingleSystemsToKet(self):
        A = MatrixSymbol('A', 3*self.n_systems, 1)
        state_out = Bosonic_Tritters.calcOutOp(self)
        for k in range(0, self.n_systems):
            i1 = 3*k
            ...
            state_out = state_out.subs(A[i1,0]**3,0).subs(A[i2,0]**3,0).subs(A[i3,0]**3,0)
            state_out = state_out.subs(A[i1,0]**2,0).subs(A[i2,0]**2,0).subs(A[i3,0]**2,0)
        if (self.n_systems == 3): return Bosonic_Tritters.toKet3(self, state_out)
        elif (self.n_systems == 2): return Bosonic_Tritters.toKet2(self, state_out)
